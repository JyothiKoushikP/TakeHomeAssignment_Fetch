...
**Assumptions**:  
As mentioned in the assignment, I have assumed a few things with regard to the design.

* The requests are independent i.e. the outcome of 1 request doesn't affect the other. I have based my approach on this assumption.
* The domain URI are of a constant format i.e. **(https:// or http:// + URI + /Endpoint)** and have got the domains on these format.
* Asynchronous API - Calls: I have assumed the requests to be independent, so I have also assumed that the calls made should not be in a order or a format. I have implemented asynchronous API Calls for the same.